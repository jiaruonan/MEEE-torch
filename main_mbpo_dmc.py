    ...
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            if cur_step >= args.epoch_length and len(env_pool) > args.min_pool_size:
                # test whether total_step matches with epoch_step
                # print('========== is1000-step: ==========', cur_step - start_step)
                break
            ...
            if cur_step > 0 and cur_step % args.model_train_freq == 0 and args.real_ratio < 1.0:
                ...
                train_predict_model(args, env_pool, predict_env, logger)  # note: 为了加速调试, 注释本句, 会对rollout_model造成bug, 因为predict_env.model.train起来才会对 self.scaler.fit(train_inputs) 进行初始化, 否则 predict_env.model.predict起来`inputs = self.scaler.transform(inputs)`会报错.
            ...
            if total_step % 1000 == 0 or total_step == 1:
...
def rollout_model(args, predict_env, agent, model_pool, env_pool, rollout_length):
    ...
    for i in range(rollout_length):
        ...
        next_states, rewards, terminals, info = predict_env.step(state, actions)
        # print(rewards[0:10, :], reward[0:10])
        # print(next_states[0, :] , next_state[0, :])
        # print(np.mean(np.sum(np.square(next_states - next_state), -1)))
